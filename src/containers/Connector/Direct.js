...
class Direct extends Component {
    ...
    onSubmitData = async (type) => {
        const {isEdit, provider, description, url, userSearchBase, username, password, endpoint, groupSearchBase, userSearchFilter, groupSearchFilter,
            port, hostname, clientId, clientSecret, token, selectedRecordIndex, runNow, name, jobName} = this.state;
    ...
    onJobRun = async (record) => {
       // const {provider, name} = record;
        /*if (provider === 'IDCS') {
            const newPayload = {runNow: true};
            await ApiService.directAuthcontroller(provider, newPayload);
        }*/
