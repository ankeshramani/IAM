...
class NewApplication extends Component {
  constructor(props) {
    super(props);
    const newState = {};
    if (props.location.pathname === '/admin/application/edit') {
    ...
    if (props.location.pathname === '/admin/application/upload') {
      newState.isMapping = false;
      newState.isUpload = true;
      newState.AppName = 'test App';
      newState.Description = 'Description'
    }
    if (props.location.pathname === '/admin/application/new') {
      newState.isMapping = false;
      newState.isNew = true;
    }
    this.state = {
  ...
  onAddMapping = () => {
    const {CSVAttribute, SystemAttribute, IsEntitlement, mapping, editIndex, custom, sysColumns} = this.state;
  ...
  onDeleteRecord = (index) => {
    ...
    this.setState({
      mapping: mapping.filter((x, i) => i !== index)
    });
  }
  ...
  render() {
    const {
      isMapping, mapping, csvColumns, sysColumns, IsEntitlement, isUpload, AppName, Description, isEdit,
      SystemAttribute, CSVAttribute, editIndex, custom, extractDate
    } = this.state;
    const csvOptionns = csvColumns.filter(x => {
      return mapping.find(y => y.CSVAttribute === x) ? false : true
    });
    const sysOptionns = sysColumns.filter(x => {
      return mapping.find(y => y.SystemAttribute === x) ? false : true
    });
              ...
              <Card>
                <CardBody>
                  ...
                  <Row className="align-items-center">
                    <Col md={3} sm={12} xs={12}><span><b>App Name</b></span></Col>
                    <Col md={9} sm={12} xs={12} className="mt-10">
                      <Input type="text" value={AppName} disabled={isUpload}/>
                    </Col>
                          ...
                          <Col>
                            <form className="form">
                              ...
                              <div className="form__form-group">
                                ...
                                <div>
                                   <span className="form__form-group-label"><b>Field Name:</b></span>
