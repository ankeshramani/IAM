...
class Users extends Component {
    ...
    GetAllIdentityUsers = async () => {
        this.setState({
            isLoading: true
        });
        const data = await ApiService.GetAllIdentityUsers();
        if (!data || data.error) {
            this.setState({
                isLoading: false
            })
    ...
    render() {
        const {isLoading, isModifyUser, selectedRecord, searchUser} = this.state;
        return(
                                ...
                                {isLoading ? <Spin className='mt-50 custom-loading'/> :
                                    <Row>
                                        <Col md={12} sm={12} xs={12}>
                                            <Table columns={this.getColumns()} size="small"
